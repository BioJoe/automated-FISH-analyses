...
# define a function to get the list of all the filename
...
def findAndMerge(filelist):
    ...
    df_out = pd.DataFrame(columns=[
        'directory', 'image', 'nul', 'single', 'multiple','genotype'])
    for file in filelist:
        file_name = ntpath.basename(file)
        dir_name = ntpath.dirname(file)
        # if a FISH results file extract the data and
        # add it to the dataframe as a new row
        if "FISH results.txt" in file_name:
            ...
            df_temp = pd.DataFrame(
                [[dirName, imagename, nul, single, multiple]],
                columns=['directory', 'image', 'nul', 'single', 'multiple'])
            df_out = df_out.append(df_temp, sort=False, ignore_index=True)
            print("It contains " +
                  str(nul) + " nuls, " +
                  str(single) + " singles and " +
                  str(multiple) + " multiple dote containing nuclei :o")
    return df_out
...
while(go_on):
    ...
    go_on = messagebox.askyesnocancel(
        title = None, message="Add another genotype?")
        
#calculate the aneuploidy ratio of spermatids
df_merged['aneuploidy ratio'] = (
    (2*df_merged['multiple'])/(df_merged['single']+2*df_merged['multiple']))

#ask the user where to save the final dataframe as .xlsx file
save_path = filedialog.asksaveasfilename(
    title='Save compiled results as ...',
    defaultextension = '.xlsx',
    initialdir = dirName,
    initialfile = "compiled chr3 FISH results")
