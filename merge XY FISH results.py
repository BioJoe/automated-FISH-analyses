...
def findAndMerge(filelist):
    ...
    df_out = pd.DataFrame(columns=[
        'directory', 'image', 'nul', 'X', 'Y', 'XY','genotype'])
    for file in filelist:
        file_name = ntpath.basename(file)
        dir_name = ntpath.dirname(file)
        # if a FISH results file extract the data
        # and add it to the dataframe as a new row
        if "FISH results.txt" in file_name:
            ...
            df_temp = pd.DataFrame(
                [[dirName, imagename, nul, X, Y, XY]],
                columns=['directory', 'image', 'nul', 'X', 'Y', 'XY'])
            df_out = df_out.append(df_temp, sort=False, ignore_index=True)
            print("It contains " + str(nul) +
                  " nuls, " + str(X) + " X, " +
                  str(Y) + " Y and " + str(XY) +
                  " XY signal containing nuclei :o")
    return df_out
...
while(go_on):
    ...
    genotype = simpledialog.askstring(
        title = None, prompt = "Enter genotype")
    df_out["genotype"] = genotype
    df_merged = df_merged.append(df_out, sort = False,
                                 ignore_index = True)
    go_on = messagebox.askyesnocancel(title = None,
                                      message="Add another genotype?")

#calculate the aneuploidy ratio of spermatids
df_merged['aneuploidy ratio'] = ((2*df_merged['XY'])/
                                 (df_merged['Y'] +
                                  df_merged['X'] +
                                  2*df_merged['XY']))
...
save_path = filedialog.asksaveasfilename(
    ...
    initialdir = dirName,
    initialfile = "compiled FISH results")
